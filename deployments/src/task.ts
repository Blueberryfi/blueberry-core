...
import { deploy, deployProxy, deploymentTxData, instanceAt } from './contracts';
...
export default class Task {
  ...
  async deployAndVerifyProxy(
    ...
    const instance = await this.deployProxy(name, args, from, force, libs);

    const { ethers } = await import('hardhat');

    const implementationSlot = '0x360894a13ba1a3210667c828492db98dca3e2076cc3735a920a3ca505d382bbc';
    const slotContents = await ethers.provider.getStorageAt(instance.address, implementationSlot);
    const implementationAddress = ethers.utils.hexStripZeros(slotContents);

    await this.verify(name, implementationAddress, [], libs);
    return instance;
  }
  ...
  async deployProxy(
    ...
    const output = this.output({ ensure: false });
    if (force || !output[name]) {
      instance = await deployProxy(this.artifact(name), args, from, libs);
      this.save({ [name]: instance });
      logger.success(`Deployed ${name} at ${instance.address}`);
