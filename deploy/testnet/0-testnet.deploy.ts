...
async function main(): Promise<void> {
  ...
  const bank = <BlueBerryBank>(
    await upgrades.deployProxy(Bank, [deployment.CoreOracle, deployment.ProtocolConfig, 2000])
  );
  ...
  const wichiFarm = <WIchiFarm>await upgrades.deployProxy(WIchiFarm, [deployment.MockIchiV2, deployment.MockIchiFarm]);
  ...
  await bank.whitelistERC1155([deployment.WERC20, deployment.MockIchiVault], true);
  await coreOracle.setRoutes(
    [deployment.MockIchiV2, ADDRESS_GOERLI.MockUSDC, deployment.MockIchiVault],
    [deployment.UniswapV3AdapterOracle, deployment.AggregatorOracle, deployment.IchiVaultOracle]
  );
  ...
  const safeBox = <SoftVault>(
    await upgrades.deployProxy(SoftVault, [ADDRESS_GOERLI.bUSDC, 'Interest Bearing USDC', 'ibUSDC'])
  );
  ...
  await bank.addBank(deployment.MockUSDC, deployment.USDC_SoftVault, deployment.USDC_HardVault, 9000);
}
