...
describe("Ichi Vault Oracle", () => {
  ...
  describe("Owner", () => {
    it("should be able to set price deviation", async () => {
      ...
      ).to.be.revertedWithCustomError(ichiOracle, "ZERO_ADDRESS");
  ...
  it("USDC/ICHI Angel Vault Price", async () => {
    ...
    const lpPrice = await ichiOracle.callStatic.getPrice(
      ADDRESS.ICHI_VAULT_USDC
    );
    ...
    const reserve1 = BigNumber.from(
      reserveData[0].mul(ichiPrice).div(BigNumber.from(10).pow(token0Decimal))
    );
    const reserve2 = BigNumber.from(
      reserveData[1].mul(usdcPrice).div(BigNumber.from(10).pow(token1Decimal))
    );
  ...
  describe("Flashloan attack test", () => {
    it("Vault Reserve manipulation", async () => {
      ...
      let lpPrice = await ichiOracle.callStatic.getPrice(
        ADDRESS.ICHI_VAULT_USDC
      );
    ...
    it("Swap tokens on Uni V3 Pool to manipulate pool reserves", async () => {
      ...
      console.log(
        "Price Deviation Config:",
        await ichiOracle.maxPriceDeviations(ICHI)
      );
      await swapRouter.exactInputSingle({
        tokenIn: USDC,
        tokenOut: ICHI,
      ...
      ).to.be.revertedWithCustomError(ichiOracle, "EXCEED_DEVIATION");
    });
  });
});
