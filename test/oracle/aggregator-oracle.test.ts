...
describe('Aggregator Oracle', () => {
	...
	beforeEach(async () => {
		...
		// Manual Oracle
		const ManualOracle = await ethers.getContractFactory(
			CONTRACT_NAMES.ManualOracle
		);
		manualOracle = <ManualOracle>await ManualOracle.deploy();
		await manualOracle.deployed();
	...
	describe('Owner', () => {
		it('should be able to set primary sources', async () => {
			...
			await expect(
				aggregatorOracle.setPrimarySources(ADDRESS.UNI, 0, [
					chainlinkOracle.address,
					bandOracle.address,
				])
			).to.be.revertedWith('OUT_OF_DEVIATION_CAP');
			...
			expect(
				await aggregatorOracle.maxPriceDeviations(ADDRESS.UNI)
			).to.be.equal(DEVIATION);
			expect(
				await aggregatorOracle.primarySourceCount(ADDRESS.UNI)
			).to.be.equal(2);
			expect(
				await aggregatorOracle.primarySources(ADDRESS.UNI, 0)
			).to.be.equal(chainlinkOracle.address);
			expect(
				await aggregatorOracle.primarySources(ADDRESS.UNI, 1)
			).to.be.equal(bandOracle.address);
		});
	...
	describe('Price Feeds', () => {
		...
		it('USDC price feeds', async () => {
			...
			const aggregatorPrice = await aggregatorOracle.getPrice(token);
			console.log(utils.formatUnits(aggregatorPrice, 18));
			expect(chainlinkPrice.add(bandPrice).div(2)).to.be.equal(
				aggregatorPrice
			);
		});

		it('alETH price feeds', async () => {
			const token = ADDRESS.alETH;
			const price = '1400';

			await manualOracle.setMultipleManualPrices(
				[token],
				[utils.parseUnits(price, 18)],
				[true]
			);

			const manualPrice = await manualOracle.getPrice(token);
			console.log('alETH Price:', utils.formatUnits(manualPrice, 18));

			const aggregatorPrice = await aggregatorOracle.getPrice(token);
			console.log(utils.formatUnits(aggregatorPrice, 18));
			expect(manualPrice).to.be.equal(aggregatorPrice);
		});
	});
});
