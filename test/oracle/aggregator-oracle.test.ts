...
describe("Aggregator Oracle", () => {
  ...
  describe("Price Feeds", () => {
    ...
    it("should revert when source prices exceed deviation", async () => {
      await aggregatorOracle.setPrimarySources(ADDRESS.ICHI, DEVIATION, [
        mockOracle1.address,
        mockOracle2.address,
      ]);
      ...
      ).to.be.revertedWithCustomError(aggregatorOracle, "EXCEED_DEVIATION");
    });
    it("should take avgerage of valid prices within deviation", async () => {
      ...
      expect(
        await aggregatorOracle.callStatic.getPrice(ADDRESS.ICHI)
      ).to.be.equal(utils.parseEther("0.98"));
      ...
      expect(
        await aggregatorOracle.callStatic.getPrice(ADDRESS.ICHI)
      ).to.be.equal(utils.parseEther("0.69"));
    });
