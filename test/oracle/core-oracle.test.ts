...
describe("Core Oracle", () => {
  ...
  describe("Owner", () => {
    it("should be able to set routes", async () => {
      await expect(
        coreOracle
          .connect(alice)
          .setRoutes([ADDRESS.USDC], [mockOracle.address])
      ...
      await expect(
        coreOracle.setRoutes([ADDRESS.USDC], [ethers.constants.AddressZero])
      ).to.be.revertedWithCustomError(coreOracle, "ZERO_ADDRESS");
    ...
    it("should revert initializing twice", async () => {
      await expect(coreOracle.initialize()).to.be.revertedWith(
  ...
  describe("Utils", () => {
    ...
    it("should to able to get if the wrapper is supported or not", async () => {
      ...
      expect(
        await coreOracle.callStatic.isWrappedTokenSupported(
          werc20.address,
          collId
        )
      ).to.be.true;
    ...
    it("should be able to get if the token price is supported or not", async () => {
      ...
      expect(await coreOracle.callStatic.isTokenSupported(ADDRESS.USDC)).to.be
        .true;
      ...
      expect(await coreOracle.callStatic.isTokenSupported(ADDRESS.ICHI)).to.be
        .false;
    });
  });
  describe("Value", () => {
  ...
  describe("Pauseable", () => {
    it("owner should be able to pause the contract", async () => {
      await expect(coreOracle.connect(alice).pause()).to.be.revertedWith(
    ...
    it("owner should be able to unpause the contract", async () => {
      ...
      await coreOracle.pause();
      await expect(coreOracle.connect(alice).unpause()).to.be.revertedWith(
        "Ownable: caller is not the owner"
      );
