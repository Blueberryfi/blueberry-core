...
contract BankConvexSpell is SpellBaseTest {
    ...
    uint256 public CURVE_FEE = 505800; // 0.005058 % approximation, might need to research more
    ...
    function testFork_BankConvexSpell_openPositionFarmSuccess() external {
        ...
        uint256 slippage = _calculateSlippage(pool, borrowAmount);
    ...
    function testForkFuzz_BankConvexSpell_openPositionGeneratesRightLPToken(
        ...
        if (collateralAmount > type(uint128).max || borrowAmount > type(uint128).max) return;
        ...
        uint256 currentDebt = bank.currentPositionDebt(1);
        uint256 collateralValue = bank.getIsolatedCollateralValue(1);
        uint256 maxLTVValue = (collateralValue * maxLTV) / DENOMINATOR;
        vm.assume(maxLTVValue > currentDebt);
        ...
        // Check if the right amount of LP landed at destination
        assertApproxEqRel(balanceAfterLP - balanceBeforeLP, slippage, 0.001e18);
    ...
    function _calculateSlippage(address pool, uint256 amount) internal view override returns (uint256) {
