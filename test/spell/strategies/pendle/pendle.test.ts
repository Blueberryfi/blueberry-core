...
import {  evm_increaseTime, evm_mine_blocks, fork, setTokenBalance } from '../../../helpers';
import SpellABI from '../../../../abi/contracts/spell/PendleSpell.sol/PendleSpell.json';
import axios from 'axios';
import { ApproxParamsStruct, BasicRouter, FixedX18, LimitOrderDataStruct, Multicall, Router, TokenInput, toAddress, toAddresses } from '@pendle/sdk-v2';
import { TokenInputStruct } from '../../../../typechain-types/contracts/interfaces/pendle-v2/IPendleRouter';
import PendleRouterABI from '../../../../abi/contracts/interfaces/pendle-v2/IPendleRouter.sol/IPendleRouter.json';
import { spell } from '../../../../typechain-types/contracts';
        ...
        for (let j = 0; j < strategyInfo.collateralAssets.length; j += 1) {
            for (let l = 0; l < strategyInfo.borrowAssets.length; l += 1) {
                describe(`Pendle Spell Test(collateral: ${strategyInfo.collateralAssets[j]} borrow: ${strategyInfo.borrowAssets[l]})`, () => {
                    before(async () => {
                        collateralToken = await ethers.getContractAt('ERC20', USDC);
...
async function generateExactTokenForPtData(requestBody: SwapExactTokenForPtBody, signer: SignerWithAddress, blockNumber: number) {

    let router = Router.getRouterWithKyberAggregator({chainId: 1, provider: signer.provider || ethers.getDefaultProvider(), signer: signer});
    ...
    let functionData = iface.encodeFunctionData('swapExactTokenForPt', contractCall.params);
...
async function generateCloseData(requestBody: SwapExactPtForTokenBody, signer: SignerWithAddress, blockNumber: number) {
    ...
    try {
        ...
        const iface = new ethers.utils.Interface(PendleRouterABI);
        let functionData = iface.encodeFunctionData('swapExactPtForToken', contractCall.params);
        ...
        let functionData = iface.encodeFunctionData('redeemPyToToken', contractCall.params);
