...
describe('ShortLong Spell mainnet fork', () => {
  ...
  let protocol: ShortLongERC4626Protocol; //ShortLongProtocol; //ShortLongERC4626Protocol;
  ...
  before(async () => {
    ...
    protocol = await setupShortLongERC4626Protocol(); //await setupShortLongProtocol(); //await setupShortLongERC4626Protocol();
    bank = protocol.bank;
  ...
  it('should be able to farm DAI', async () => {
    ...
    const swapData = await getParaswapCalldata(CRV, PxETH, borrowAmount, spell.address, 100);
