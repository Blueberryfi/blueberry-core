...
export const setupShortLongERC4626Protocol = async (): Promise<ShortLongERC4626Protocol> => {
  ...
  let hardVaultOracle: ERC4626Oracle; //TODO: create a derived contract for this called HardVaultOracle that extends ERC4626Oracle
  let oracle: CoreOracle;
  let shortLongSpell: ShortLongSpell_ERC4626;
  ...
  let apxETHHardVault: HardVault;
  let apxETHSoftVault: SoftVault;
  ...
  hardVaultOracle = <ERC4626Oracle>await upgrades.deployProxy(
  ...
  await hardVaultOracle.deployed();
  ...
  apxETHSoftVault = <SoftVault>await upgrades.deployProxy(
    SoftVault,
    [config.address, bApxETH.address, 'Auto Compounding pxETH', 'ibApxETH', admin.address],
    {
      unsafeAllow: ['delegatecall'],
    }
  );
  await apxETHSoftVault.deployed();

  apxETHHardVault = <HardVault>await upgrades.deployProxy(HardVault, [config.address, admin.address], {
    unsafeAllow: ['delegatecall'],
  });
  await apxETHHardVault.deployed();
  ...
  await softVaultOracle.registerSoftVault(apxETHSoftVault.address);
  await hardVaultOracle.registerToken(ApxETH);
  ...
  await oracle.setRoutes(
    [
      ...
      apxETHSoftVault.address,
      apxETHHardVault.address,
    ],
    [
  ...
  await shortLongSpell.addStrategy(apxETHHardVault.address, MIN_POS_SIZE, MAX_POS_SIZE);
  await shortLongSpell.setCollateralsMaxLTVs(
    5,
    [WBTC, DAI, WETH, USDC, WstETH, ApxETH],
    [MAX_LTV, MAX_LTV, MAX_LTV, MAX_LTV, MAX_LTV, MAX_LTV]
  );
  ...
  await bank.addBank(ApxETH, apxETHSoftVault.address, apxETHHardVault.address, 9000); //todo: might need to create a softvault for the undelrying token of apxETH
  ...
  const apxETHDeposit = (parseInt(strategyDepositInUsd) / ETH_PRICE).toFixed(18).toString();
  console.log('apxETH Deposit:', apxETHDeposit, await apxETH.balanceOf(admin.address));
  await apxETH.approve(apxETHHardVault.address, ethers.constants.MaxUint256);
  await apxETHHardVault.deposit(ApxETH, utils.parseUnits(apxETHDeposit, 18));
