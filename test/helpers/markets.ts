...
export const getMarketInfo = async (bToken: BErc20Delegator): Promise<MarketInfo> => {
  const underlyingAddress = await bToken.underlying();
  const marketInfo: MarketInfo | undefined = blueberryMarkets.find(
    (market) => market.underlyingAddress === underlyingAddress
  );
...
export const deploySoftVaults = async (
  config: any,
  bank: any,
  comptroller: any,
  ...
  for (let i = 0; i < bTokens.length; i++) {
    const bToken: BErc20Delegator = bTokens[i];
    const bTokenInfo = await getMarketInfo(bToken);
    if ((await bank.isTokenWhitelisted(bTokenInfo.underlyingAddress)) != true) {
      continue;
    }

    const softVault = <SoftVault>await upgrades.deployProxy(
      SoftVault,
      [
        config.address,
        bToken.address,
        `Interest Bearing ${bTokenInfo.underlyingName}`,
        `ib${bTokenInfo.underlyingName}`,
        admin.address,
      ],
      {
        unsafeAllow: ['delegatecall'],
    ...
    const liqThreshold = bTokenInfo.isUnderlyingStable ? 9000 : 8500;
...
export const getSoftVault = async (softVaults: SoftVault[], underlyingToken: string): Promise<SoftVault> => {
  const softVault = softVaults.find(async (softVaults) => (await softVaults.getUnderlyingToken()) === underlyingToken);
