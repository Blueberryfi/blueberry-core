...
export const setupCrvProtocol = async (): Promise<CrvProtocol> => {
  ...
  let bTokens = await deployBTokens(admin.address, oracle.address, [
    { token: ADDRESS.USDT, symbol: "USDT" },
  ]);
  ...
  await bank.whitelistTokens([USDC, CRV, DAI, USDT], [true, true, true, true]);
  ...
  usdtSoftVault = <SoftVault>(
    await upgrades.deployProxy(
      SoftVault,
      [config.address, bUSDT.address, "Interest Bearing USDT", "ibUSDT"],
      { unsafeAllow: ["delegatecall"] }
    )
  );
  await usdtSoftVault.deployed();
  await bank.addBank(USDT, usdtSoftVault.address, hardVault.address, 8500);
