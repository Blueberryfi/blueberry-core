    ...
    IWCurveGauge
{
    ...
    function decodeId(
        uint id
    ) public pure returns (uint pid, uint gid, uint crvPerShare) {
    ...
    /// @param pid Pool id
    /// @param gid Gauge id
    function registerGauge(uint pid, uint gid) external onlyOwner {
    ...
    function burn(uint id, uint amount) external nonReentrant returns (uint) {
        ...
        uint stCrv = (stCrvPerShare * amount) / 1e18;
        uint enCrv = (gauge.accCrvPerShare * amount) / 1e18;
        if (enCrv > stCrv) {
