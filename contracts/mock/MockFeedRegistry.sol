...
contract MockFeedRegistry is Ownable {
    mapping(address => mapping(address => address)) _feeds;
    ...
    function setFeed(address base, address quote, address aggregator) external onlyOwner {
    ...
    function _getFeed(address base, address quote) internal view returns (AggregatorV2V3Interface aggregator) {
        aggregator = AggregatorV2V3Interface(_feeds[base][quote]);
    }
}
